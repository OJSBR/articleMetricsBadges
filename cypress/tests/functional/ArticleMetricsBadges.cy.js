...
describe('Article Metrics Badges plugin', function() {
	...
	const findArticleWithDoi = () => cy.window().then((win) => cy.wrap((async() => {
		...
		for (const submission of list.items || []) {
			...
			// OJS 3.4 and 3.5 give the DOI object, OJS 3.3 the pub-id.
			const doi = (publication.doiObject && publication.doiObject.doi) || publication['pub-id::doi'];
			if (doi) {
				return {id: submission.id, doi: doi};
			}
		}
		return null;
	})(), {timeout: 60000}));
